...
import {
  ...
  ToOrFromType,
  DefaultCurrencies, BuySupportedChains, BuyOption, BuyServiceId
...
function Buy (props: Props) {
  ...
  React.useEffect(() => {
    ...
    if (isAvailableOnRamp && !isAvailableOnWyre) {
      setBuyOptions(BuyOptions.filter(option => option.id !== 'wyre'))
    }

    if (!isAvailableOnRamp && isAvailableOnWyre) {
      setBuyOptions(BuyOptions.filter(option => option.id !== 'ramp'))
    }
  ...
  return (
    <StyledWrapper>
      ...
      >
        {selectedBuyOption === 'ramp' ? <RampLogo /> : <WyreLogo />}
        {buyWithLabel}
      </BuyWithButton>
      <Spacer />
      <NavButton
